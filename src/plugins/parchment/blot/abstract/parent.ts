...
class ParentBlot extends ShadowBlot implements Parent {
  ...
  public descendants(
    criteria: (blot: Blot) => boolean, // LeafBlot
    index: number,
    length: number,
  ): Blot[];
  public descendants(
    criteria: any, // LeafBlot
    index = 0,
    length: number = Number.MAX_VALUE,
  ): Blot[] {
    let descendants: Blot[] = []; // LeafBlot
    ...
    this.children.forEachAt(
      ...
      (child: Blot, childIndex: number, childLength: number) => {
        if (
          (criteria.blotName == null && criteria(child)) ||
          (criteria.blotName != null && child instanceof criteria) // 找出所有的子节点
        ) {
